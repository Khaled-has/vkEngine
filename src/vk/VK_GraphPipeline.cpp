...
void VK_GraphPipeline::Create(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount)
{
	CreateDescriptorPool(mImageCount, pBindings, mBindingsCount);
	CreateDescriptorLayout(mImageCount, pBindings, mBindingsCount);
	AllocateDescriptorSets(mImageCount);
	UpdateDescriptorSets(mImageCount, pBindings, mBindingsCount);
}
...
void VK_GraphPipeline::CreatePipeline(const VK_Shader& pShader, const VK_Attachment* pAttachs, uint32_t mAttachCount, VkViewport mViewport, VkRect2D mScissor)
{
	...
	for (uint32_t i = 0; i < mAttachCount; i++)
	{
		if (pAttachs[i].mIsColorAttach)
		{
			ColorFormats.push_back(pAttachs[i].mFormat);
		}
		else if (pAttachs[i].mIsDepthAttach && IsHaveDepthAttach)
		{
			LOG_WARN("You have tow depth attachment should you have only one: We use the first DepthAttachment");
		}
		else if (pAttachs[i].mIsDepthAttach)
		{
			IsHaveDepthAttach = true;
			DepthFormat = pAttachs[i].mFormat;
		}
	}
...
void VK_GraphPipeline::CreateDescriptorPool(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount)
{
	std::vector<VkDescriptorPoolSize> PoolSizes;

	for (uint32_t i = 0; i < mBindingsCount; i++)
	{
		PoolSizes.push_back(
			VkDescriptorPoolSize{
				.type = pBindings[i].mType,
...
void VK_GraphPipeline::CreateDescriptorLayout(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount)
{
	...
	for (uint32_t i = 0; i < mBindingsCount; i++)
	{
		Bindings.push_back(
			VkDescriptorSetLayoutBinding{
				.binding = pBindings[i].mBind,
				.descriptorType = pBindings[i].mType,
				.descriptorCount = pBindings[i].mDescCount,
				.stageFlags = pBindings[i].mStage
			}
		);
	}
...
void VK_GraphPipeline::UpdateDescriptorSets(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount)
{
	for (uint32_t i = 0; i < mImageCount; i++)
	{
		std::vector<VkWriteDescriptorSet> WriteDescSets{};

		for (uint32_t j = 0; j < mBindingsCount; j++)
		{
			// # Buffers
			if (pBindings[j].mBuffer != nullptr && pBindings[j].mBufCount > 0)
			{
				VkDescriptorBufferInfo BufferInfo = {
					.buffer = pBindings[j].mBufCount > 1 ? pBindings[j].mBuffer[i].getBuffer() : pBindings[j].mBuffer->getBuffer(),
					.offset = 0,
					.range = VK_WHOLE_SIZE
				};
				...
				WriteDescSets.push_back(
					VkWriteDescriptorSet{
						...
						.dstBinding = pBindings[j].mBind,
						.dstArrayElement = 0,
						.descriptorCount = pBindings[j].mDescCount,
						.descriptorType = pBindings[j].mType,
						.pBufferInfo = &BufferInfo,
					}
				);
			}
			// # Texture
			else if (pBindings[j].mTexture != nullptr && pBindings[j].mTexCount > 0)
			{
				const VK_Texture* CurTexture = pBindings[j].mTexCount > 1 ? &pBindings[j].mTexture[i] : pBindings[j].mTexture;
