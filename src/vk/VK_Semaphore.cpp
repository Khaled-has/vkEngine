...
void VK_SemaphoresManager::Create(uint32_t mFrameCount)
{
	...
	m_pSemaphores.resize(mFrameCount);
...
VkResult VK_SemaphoresManager::AcquireNextImage(const VK_SwapChain& mSwapChain)
{
	...
	vkWaitForFences(Device, 1, &m_pSemaphores[mFrameIndex].mInFlightFence, VK_TRUE, UINT64_MAX);
	vkResetFences(Device, 1, &m_pSemaphores[mFrameIndex].mInFlightFence);

	VkResult res = vkAcquireNextImageKHR(
		Device, pSwapChain,
		UINT64_MAX, m_pSemaphores[mFrameIndex].mImageAvailableSem,
		VK_NULL_HANDLE, &mAcquiredImageIndex
	);

	if ((m_pSemaphores[mAcquiredImageIndex].mImageInFlightFence != VK_NULL_HANDLE) &&
		(m_pSemaphores[mAcquiredImageIndex].mImageInFlightFence != m_pSemaphores[mFrameIndex].mInFlightFence))
	{
		vkWaitForFences(Device, 1, &m_pSemaphores[mAcquiredImageIndex].mImageInFlightFence, VK_TRUE, UINT64_MAX);
	}
...
void VK_SemaphoresManager::SubmitAsync(const VK_CmdBuf* CmdBuf, uint32_t Count, const VK_Queue& mQueue)
{
	...
	VkSubmitInfo SubmitInfo = {
		...
		.pWaitSemaphores = &m_pSemaphores[mFrameIndex].mImageAvailableSem,
		.pWaitDstStageMask = waitFlags,
		.commandBufferCount = Count,
		.pCommandBuffers = &CmdBuf->getHandle(),
		.signalSemaphoreCount = 1,
		.pSignalSemaphores = &m_pSemaphores[mFrameIndex].mRenderFinishedSem
	};

	CHECK_VK_RES(
		vkQueueSubmit(mQueue.getHandle(), 1, &SubmitInfo, m_pSemaphores[mFrameIndex].mInFlightFence), 
		"vkQueueSubmit"
	);
}
...
VkResult VK_SemaphoresManager::Present(uint32_t mImageIndex, const VK_SwapChain& mSwapChain, const VK_Queue& mQueue)
{
	VkPresentInfoKHR PresentInfo = {
		...
		.pNext = NULL,
		.waitSemaphoreCount = 1,
		.pWaitSemaphores = &m_pSemaphores[mFrameIndex].mRenderFinishedSem,
		.swapchainCount = 1,
