...
public:
	...
	void Create(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount);
	void Destroy();
	
	void CreatePipeline(
		const VK_Shader& pShader, const VK_Attachment* pAttachs, 
		uint32_t mAttachCount, VkViewport mViewport, VkRect2D mScissor
	);
...
private:
	...
	void CreateDescriptorPool(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount);
	void CreateDescriptorLayout(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount);
	void AllocateDescriptorSets(uint32_t mImageCount);
	void UpdateDescriptorSets(uint32_t mImageCount, const VK_Binding* pBindings, uint32_t mBindingsCount);
};
