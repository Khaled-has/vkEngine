...
#endif
	Window::getInstance()->CreateVkSurface(m_pInstance, &m_pSurface);
	EnumeratePhysicalDevices();
	CreateDevice();
	CreateAllocator();

	CreateSimpleOperationsCmdBuf();
...
void VK_Device::CreateSimpleOperationsCmdBuf()
{
	m_CmdPool.Create(m_pQueues.mTransfer->getQFamilyIndex());
	m_CmdPool.AllocCmdBufs(1, &m_SimpleOperationCmdBuf);
}

void VK_Device::DestroySimpleOperationsCmdBuf()
{
	m_CmdPool.FreeCmdBufs(1, &m_SimpleOperationCmdBuf);
	m_CmdPool.Destroy();
}